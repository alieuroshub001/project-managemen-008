...
export default function LeaveReviewForm({ requestId }: Props) {
  ...
  const handleSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        router.push(`/dashboard/hr/leave/${id}`);
      } else {
  ...
  return (
    <Card
      ...
      extra={<Button onClick={() => router.push(`/dashboard/hr/leave/${id}`)}>Back to Request</Button>}
    >
      {leaveRequest && (
        <div className="mb-6">
      ...
      <Form form={form} layout="vertical" onFinish={handleSubmit}>
        ...
        <Form.Item>
          ...
          <Button className="ml-2" onClick={() => router.push(`/dashboard/hr/leave/${id}`)}>
            Cancel
          </Button>
        </Form.Item>
      </Form>
