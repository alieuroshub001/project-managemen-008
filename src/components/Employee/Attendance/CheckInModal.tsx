...
interface CheckInModalProps {
  ...
  existingRecord?: any; // Pass existing record if any
}
...
export default function CheckInModal({ 
  ...
  const handleSubmit = async (values: any, forceCheckIn: boolean = false) => {
    setLoading(true);
    try {
      const payload = {
  ...
  const handleConfirmCheckIn = () => {
    form.validateFields().then((values) => {
      handleSubmit(values, true); // Force check-in
    });
  };
  ...
  return (
    <Modal
      ...
      width={600}
    >
      <Form form={form} onFinish={(values) => handleSubmit(values, false)} layout="vertical">
        {existingRecord && (
          <Alert
