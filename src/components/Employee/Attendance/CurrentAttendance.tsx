...
import { Card, Typography, Button, Space, Tag, Statistic, Row, Col, Alert, Spin, message, Divider, List } from 'antd';
import { 
  ClockCircleOutlined, 
  PlayCircleOutlined, 
  PauseCircleOutlined,
...
import { format, parseISO, differenceInMinutes, isToday, startOfDay } from 'date-fns';
...
export default function CurrentAttendance({ 
  ...
  const handleBreakAction = async (action: 'start' | 'end', breakType?: string, breakNotes?: string) => {
    ...
    try {
      ...
      message.error(error.message || `Failed to ${action} break`);
    } finally {
  ...
  const handleNamazAction = async (action: 'start' | 'end', namazType?: string) => {
    ...
    try {
      ...
      message.error(error.message || `Failed to ${action} namaz`);
    } finally {
      setActionLoading(null);
    }
  };
