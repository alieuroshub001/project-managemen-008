...
interface NamazModalProps {
  visible: boolean;
  onCancel: () => void;
  onAction: (action: 'start' | 'end', namazType?: string) => void;
...
export default function NamazModal({ 
  ...
  const handleStartNamaz = async (values: any) => {
    setLoading(true);
    try {
      await onAction('start', values.namazType);
    } catch (error) {
      console.error('Start namaz failed:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEndNamaz = async () => {
    ...
    try {
      ...
      console.error('End namaz failed:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (isNamazActive) {
    return (
      ...
      >
        <Text>
          Are you sure you want to end your current prayer time?
        </Text>
      </Modal>
    );
  }
  ...
  return (
    <Modal
      title="Start Prayer Time"
      open={visible}
      onCancel={onCancel}
      footer={null}
    >
      <Form form={form} onFinish={handleStartNamaz} layout="vertical">
        <Text className="block mb-4">
          Are you sure you want to start prayer time?
        </Text>
