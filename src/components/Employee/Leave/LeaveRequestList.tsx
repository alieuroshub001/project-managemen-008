...
export default function LeaveRequestList({ filter, role }: Props) {
  ...
  const canCreate = role ? role === 'employee' : true;

  return (
    <div className="space-y-4">
      {/* Header actions only on the "All" tab and if allowed by role */}
      {!filter?.status && (
